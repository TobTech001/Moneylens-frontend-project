...
export default function SubscriptionActionsMenu({ status, onEdit, onPause, onCancel, onDelete }: SubscriptionActionsMenuProps) {
    ...
    <div ref={ref} className="relative">
      ...
      {open && (
        ...
        >
          ...
          {status === 'active' && (
            <button role="menuitem" type="button" onClick={() => handle(onPause)} className="block w-full px-3.5 py-2.5 text-left text-sm text-warning hover:bg-warning/10">
              Pause
            </button>
          )}
          {(status === 'active' || status === 'paused') && (
            <button role="menuitem" type="button" onClick={() => handle(onCancel)} className="block w-full px-3.5 py-2.5 text-left text-sm text-ink hover:bg-surface-alt">
              Cancel
            </button>
          )}
